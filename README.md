...
At runtime, the production app stack requires the following environment variables to be present.

| **Variable**                 | **Description**                                                                         |
| ---------------------------- | --------------------------------------------------------------------------------------- |
| `HOST`                       | HAProxy authority for ingress host matching (e.g., `drap.dcs.upd.edu.ph`).              |
| `PUBLIC_ORIGIN`              | Canonical public origin for the app and meta tags (e.g., `http://drap.dcs.upd.edu.ph`). |
| `ORIGIN`                     | Server origin, set equal to `PUBLIC_ORIGIN` in Compose.                                 |
...
| `DRAP_ASSERT_DOMAIN`         | Assert domain for incoming users. Unrestricted by default.                              |
...
> [!IMPORTANT]
> The OAuth redirect URI is computed as `${ORIGIN}/dashboard/oauth/callback`, where `ORIGIN` is set to the public HAProxy-facing origin via `PUBLIC_ORIGIN`.
...
# requires `HOST` and `PUBLIC_ORIGIN`
pnpm docker:app
```
